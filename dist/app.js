...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('.sidebar-item-wrapper').forEach(wrapper => {
        ...
        wrapper.addEventListener('mouseenter', () => {
            ...
            if (btn.classList.contains('logout-btn'))
                tooltip.classList.add('logout-tooltip');
    ...
    if (roleModal)
        roleModal.addEventListener('click', (e) => { if (e.target === roleModal)
            closeModalFn(); });
    // ══════════════════════════════════════════════════════════════
    //  RENDER FUNCTIONS
    // ══════════════════════════════════════════════════════════════
    ...
    function renderRevenueChart() {
        ...
        if (container && DATA.revenueByMonth) {
            const values = DATA.revenueByMonth.map(d => d.value);
    ...
    function renderTransactions(filter = '', statusFilter = '', methodFilter = '') {
        ...
        if (tbody) {
            ...
            if (methodFilter)
                txns = txns.filter(t => t.method === methodFilter);
    ...
    function renderProducts(filter = '', catFilter = '', stockFilter = '') {
        ...
        if (tbody) {
            ...
            if (stockFilter)
                prods = prods.filter(p => p.status === stockFilter);
      ...
      `).join('');
            ...
            if (goodCount)
                goodCount.textContent = DATA.products.filter(p => p.status === 'good').length.toString();
            const lowCount = document.getElementById('lowStockCount');
            if (lowCount)
                lowCount.textContent = DATA.products.filter(p => p.status === 'low').length.toString();
            const criticalCount = document.getElementById('criticalStockCount');
            if (criticalCount)
                criticalCount.textContent = DATA.products.filter(p => p.status === 'critical').length.toString();
        }
    }
    ...
    if (userSearch) {
        userSearch.addEventListener('input', (e) => {
            renderUsers(e.target.value, roleFilter ? roleFilter.value : '');
        });
    }
    if (roleFilter) {
        roleFilter.addEventListener('change', (e) => {
            renderUsers(userSearch ? userSearch.value : '', e.target.value);
        });
    }
    // Transactions
    const txnSearch = document.getElementById('txnSearch');
    const txnStatusFilter = document.getElementById('txnStatusFilter');
    const txnMethodFilter = document.getElementById('txnMethodFilter');
    function applyTxnFilters() {
        renderTransactions(txnSearch ? txnSearch.value : '', txnStatusFilter ? txnStatusFilter.value : '', txnMethodFilter ? txnMethodFilter.value : '');
    }
    if (txnSearch)
        txnSearch.addEventListener('input', () => applyTxnFilters());
    if (txnStatusFilter)
        txnStatusFilter.addEventListener('change', () => applyTxnFilters());
    if (txnMethodFilter)
        txnMethodFilter.addEventListener('change', () => applyTxnFilters());
    // Products
    const productSearch = document.getElementById('productSearch');
    const categoryFilter = document.getElementById('categoryFilter');
    const stockFilter = document.getElementById('stockFilter');
    function applyProductFilters() {
        renderProducts(productSearch ? productSearch.value : '', categoryFilter ? categoryFilter.value : '', stockFilter ? stockFilter.value : '');
    }
    if (productSearch)
    ...
    if (empSearch) {
        empSearch.addEventListener('input', (e) => {
            renderEmployees(e.target.value, deptFilter ? deptFilter.value : '');
        });
    }
    if (deptFilter) {
        deptFilter.addEventListener('change', (e) => {
            renderEmployees(empSearch ? empSearch.value : '', e.target.value);
        });
    }
    // ══════════════════════════════════════════════════════════════
    //  CSV UTILITIES
    ...
    const importTxnFile = document.getElementById('importTxnFile');
    if (btnImportTxn && importTxnFile) {
        btnImportTxn.addEventListener('click', () => {
            importTxnFile.click();
        });
        importTxnFile.addEventListener('change', (e) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const importProductFile = document.getElementById('importProductFile');
    if (btnImportProducts && importProductFile) {
        btnImportProducts.addEventListener('click', () => {
            importProductFile.click();
        });
        importProductFile.addEventListener('change', (e) => __awaiter(void 0, void 0, void 0, function* () {
            const target = e.target;
            const file = target.files ? target.files[0] : null;
            if (!file)
                return;
    ...
    const importStoreFile = document.getElementById('importStoreFile');
    if (btnImportStores && importStoreFile) {
        btnImportStores.addEventListener('click', () => {
            importStoreFile.click();
        });
        importStoreFile.addEventListener('change', (e) => __awaiter(void 0, void 0, void 0, function* () {
            const target = e.target;
            const file = target.files ? target.files[0] : null;
            if (!file)
                return;
    ...
    if (btnImportEmployees && importEmpFile) {
        ...
        importEmpFile.addEventListener('change', (e) => __awaiter(void 0, void 0, void 0, function* () {
            const target = e.target;
            const file = target.files ? target.files[0] : null;
            if (!file)
                return;
    ...
    function openEditUserModal(userId) {
        ...
        const editUserStore = document.getElementById('editUserStore');
        ...
        if (editUserStore)
            editUserStore.value = user.store;
        // Populate roles
        if (roleSelect) {
    ...
    if (modalEditUser)
        modalEditUser.addEventListener('click', (e) => { if (e.target === modalEditUser)
            closeEditUserModal(); });
    if (formEditUser) {
        formEditUser.addEventListener('submit', (e) => {
            ...
            if (userIndex > -1) {
                DATA.users[userIndex] = Object.assign(Object.assign({}, DATA.users[userIndex]), { name, email, role, store });
                const userSearch = document.getElementById('userSearch');
                const roleFilter = document.getElementById('roleFilter');
    ...
    function closeAllDropdowns() {
        if (dropdownNotifications)
            dropdownNotifications.classList.remove('active');
        if (dropdownProfile)
    ...
    if (btnNotifications && dropdownNotifications) {
        btnNotifications.addEventListener('click', (e) => {
            ...
            closeAllDropdowns();
            if (!isActive)
                dropdownNotifications.classList.add('active');
        });
    ...
    if (btnHelp && dropdownHelp) {
        btnHelp.addEventListener('click', (e) => {
            ...
            closeAllDropdowns();
            if (!isActive)
                dropdownHelp.classList.add('active');
        });
    ...
    function openEditProductModal(sku) {
        ...
        const prodReorder = document.getElementById('prodReorder');
        if (prodName)
        ...
        if (prodPrice)
            prodPrice.value = String(product.price);
        if (prodStock)
            prodStock.value = String(product.stock);
        if (prodReorder)
            prodReorder.value = String(product.reorderLevel);
        // Set Edit Mode
        if (modalTitle)
        ...
        if (formAddProduct)
            formAddProduct.dataset.editSku = sku;
        modalAddProduct.classList.add('active');
    }
    ...
    if (btnAddProduct && modalAddProduct) {
        ...
        if (formAddProduct) {
            formAddProduct.addEventListener('submit', (e) => {
                ...
                let status = 'good';
                if (stock === 0)
                    status = 'critical'; // changed from Out of Stock to critical/low/good to match type
                ...
                if (editSku) {
                    ...
                    if (index !== -1) {
                        DATA.products[index] = Object.assign(Object.assign({}, DATA.products[index]), { name: prodName ? prodName.value : '', sku: prodSku ? prodSku.value : '', category: prodCategory ? prodCategory.value : '', price: parseFloat(prodPrice ? prodPrice.value : '0'), stock: stock, reorderLevel: reorder, status: status });
        ...
        if (formAddEmployee) {
            formAddEmployee.addEventListener('submit', (e) => {
                ...
                const newEmployee = {
                    ...
                    status: (empStatus ? empStatus.value : 'active'),
